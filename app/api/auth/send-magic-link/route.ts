...
export async function POST(req: NextRequest) {
  try {
    ...
    // Email is best-effort — DB save above already protects the session
    if (!process.env.RESEND_API_KEY) {
      console.warn('RESEND_API_KEY not set — magic link stored but email not sent:', link)
    ...
    try {
      ...
      console.error('Resend failed — link stored, email not sent:', emailErr)
      return NextResponse.json({ success: true, email_sent: false })
    }
